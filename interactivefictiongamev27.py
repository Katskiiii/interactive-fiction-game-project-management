...
def handle_math_quiz():
    ...
    tk.Label(quiz_window, text="Answer these math questions:").pack()
    ...
    def check_math_answers():
        ...
        if None in selected_answers:
            message_label.config(text="Please answer all the questions and try again.")
            root.update_idletasks()
            root.after(2000, lambda: message_label.config(text=""))
        else:
            ...
            if all(selected_answer == question["correct_answer"] for selected_answer, question in zip(selected_answers, MATH_QUIZ_QUESTIONS)):
                ...
                if stamina_points <= 0:
                    ...
                    message_label.config(text="Oh no you got a question wrong! You lost 20 stamina. Let's restart", fg="red")
                    root.update_idletasks()
                    root.after(2000, lambda: message_label.config(text=""))
...
def handle_science_quiz():
    ...
    # Prompt the player to answer the questions
    tk.Label(quiz_window, text="Answer these science questions:").pack()
        ...
        if None in selected_answers:
            ...
            if all(selected_answer == question["correct_answer"] for selected_answer, question in zip(selected_answers, SCIENCE_QUIZ_QUESTIONS)):
                ...
                if stamina_points <= 0:
                    ...
                    message_label.config(text="Oh no you got a question wrong! You lost 10 stamina. Let's restart", fg="red")
                    root.update_idletasks()
                    root.after(2000, lambda: message_label.config(text=""))
                    # Clear the selected answers
...
def handle_quiz():
    ...
    # Prompt the player to answer the questions
    tk.Label(quiz_window, text="Answer these art questions:").pack()
...
def handle_secret_room_quiz():
    ...
    # Prompt the player to answer the questions
    tk.Label(quiz_window, text="Answer these history questions:").pack()
